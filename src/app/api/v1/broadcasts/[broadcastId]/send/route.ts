...
import { handleSendBroadcastRequest } from "@/lib/broadcast-http";
...
export async function POST(request: Request, context: BroadcastRouteContext) {
  ...
  return withApiLog(request, dependencies, async (scoped) => handleSendBroadcastRequest(request, broadcastId, scoped));
}
